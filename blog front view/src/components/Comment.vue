<template>
  <div class="container">
    <h3 class="header">Comments | Come and comment</h3>
        ...
        <transition name="fade">
          <div class="input-wrapper" v-if="showItemId === item.comment_id">
            ...
            <div class="btn-control">
              <el-button class="cancel" type="info" plain round @click="cancel">cancel</el-button>
              <el-button class="btn" type="primary" round @click="commitComment">determine</el-button>
            </div>
          </div>
        </transition>
      </div>
    ...
    <div class="fatherInput">
      <div class="input-wrapper" v-if="showFatherInput">
        ...
        <div class="input-control">
          ...
          <el-input
            class="input"
            placeholder="get email and your image"
            prefix-icon="el-icon-message"
            type="email"
            v-model="commentForm.email"
          ></el-input>
      ...
      if (this.commentForm.blog_id != null) {
        ...
        this.$axios.post("/comment/save", _this.commentForm).then(res => {
          ...
          if (res.data.code === 200) {
            ...
            _this.commentForm.email = "";
            _this.commentForm.nick_name = "";
            _this.$message.success(res.data.msg);
          } else {
            this.$message.warning("please refresh page then leave the comment");
          }
        });
        }else{
          this.$message.warning("Check for completeness");
        }

      } else {
        this.$message.warning("Please refresh the page before commenting");
      }
    }
  },
  mounted() {}
